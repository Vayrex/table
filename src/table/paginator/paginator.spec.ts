...
describe("Paginator", () => {
  ...
  describe("Initialization", () => {
    ...
    test("Page can not be negative or zero", () => {
      expect(() => {
        new Paginator(0, 10, 0);
      }).toThrow();
    });

    test("Page must not be greater then total number of pages", () => {
      expect(() => {
        new Paginator(1, 10, 4);
      }).toThrow();
    });

    test("Records per page can not be negative or zero", () => {
